...
(() => {
  ...
  const saveDrawingFromResultButton = document.getElementById("saveDrawingFromResultButton");
  ...
  const translations = {
    zh: {
      ...
      privacyTitle: "本機隱私模式",
      privacyText: "繪圖與分析只在您的瀏覽器中進行，不會上傳、儲存或提供給第三方。",
      ...
      saveArtDone: "藝術作品已儲存／分享完成。",
      saveDrawingDone: "原始手繪圖已儲存／分享完成。",
      saveCancelled: "尚未儲存；可再按一次儲存按鈕。",
      saveFailed: "無法自動儲存，請截圖或長按影像儲存。",
    ...
    en: {
      ...
      privacyText: "Drawing and analysis happen only in your browser and are not uploaded, stored, or shared with third parties.",
      ...
      saveArtDone: "The artwork has been saved / shared.",
      saveDrawingDone: "The raw drawing has been saved / shared.",
      saveCancelled: "Nothing was saved. You may tap the save button again.",
      saveFailed: "Unable to save automatically. Please take a screenshot or long-press the image to save it.",
  ...
  async function saveCanvas(canvas, filename, doneKey) {
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        saveNotice.textContent = t(doneKey);
        return;
      }
  ...
  saveArtButton.addEventListener("click", () => saveCanvas(artCanvas, `ai-mind-artwork-${Date.now()}.png`, "saveArtDone"));
  saveDrawingButton.addEventListener("click", () => saveCanvas(drawingCanvas, `ai-mind-raw-drawing-${Date.now()}.png`, "saveDrawingDone"));
  saveDrawingFromResultButton.addEventListener("click", () => saveCanvas(drawingCanvas, `ai-mind-raw-drawing-${Date.now()}.png`, "saveDrawingDone"));
